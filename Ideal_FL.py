...
logdir = f"logs/test/pureFL/{int(time.time())}/"
...
def sum_scaled_weights(scaled_weight_list,noise):
    ...
    for grad_list_tuple in zip(*scaled_weight_list):
        ...
        # print("layer_mean",b )
        noise = 0 
        avg_grad.append(layer_mean + noise)
        b=avg_grad
        # print(b)
        # i = i+1
...
def test_model(X_test, Y_test,  model, comm_round):
    ...
    with test_summary_writer.as_default():
       tf.summary.scalar('ACCURACY', acc, step=comm_round)
       tf.summary.scalar('LOSS', loss, step=comm_round)
    return acc, loss   
...
for comm_round in range(comms_round):
    ...
    nrow=len(global_weights)
    ncol=len(global_weights[0])
